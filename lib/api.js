                ...
                if (pending.length === 0) {
                    ...
                    redisClient.multi(redisCommands).exec(function(error, replies) {
                        ...
                        if (Object.keys(donations).length) {
                            for (var wallet in donations) feePercent += donations[wallet] / 100;
                        }
                        ...
                        for (var i = 0; i < replies.length; i += 2) {
                            ...
                            var reward = Math.round(block.reward * removeFees * myScore / totalScore);
                            pending_scores.push({
                                height: block.height,
                                hash: block.hash,
                                time: block.time,
...
function handleMinerStats(urlParts, response){
    ...
    if (longpoll){
        ...
        collectWorkerStats(address, function(data) {
            var dataJSON = JSON.stringify(data);
