        ...
        function(blocks, callback){
            ...
            blocks.forEach(function(block){
                ...
                for (var worker in block.workerShares) {
                    ...
                    var data = [block.height, block.hash, block.difficulty, 'orphan', 'orphan', block.workerShares[worker], totalShares];
            ...
            blocks.forEach(function(block){
                ...
                if (block.workerScores) {
                    ...
                    Object.keys(block.workerScores).forEach(function (worker) {
                        ...
                        //     HEIGHT:HASH:DIFF:TOTALREWARD:WORKERREWARD:WORKERSHARES:TOTALSHARES[:WORKERSCORE:TOTALSCORE]
                        // (the last two are only present if score != shares, i.e. under slush rewards)
                        var data = [block.height, block.hash, block.difficulty, block.reward, workerReward, block.workerShares[worker], totalShares];
